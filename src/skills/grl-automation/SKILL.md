...
| --- | --- | --- |
| Sviluppo software | BMM Analyst/PM/Architect/Dev/TEA + Otto, Kai, Bruno, Enzo | `gre-board`, test, sicurezza, rollback |
| Database e persistenza | Dario + BMM Architect/Dev/TEA | glossario di dominio, ricerca live, benchmark, migrazione reversibile e `gre-board` |
...
| Sito WordPress, contenuti e consegna | Milo + Iris, Nora, Kai, Bruno secondo il segnale | `grl-wordpress-delivery`, modello contenuti approvato, media con attachment ID, autorizzazione esplicita e release gate di `gre-board` |
...
passo `gre-profile:domain` oppure marca il termine come `da confermare`: non trasformare una
...
| Se emerge | Passa a |
| --- | --- |
| codice, requisiti, test e release software | BMM e `gre-board` |
...
| modello dati, motore, schema, query, migrazione o recovery del datastore | Dario e `grl-agent-database`; `gre-profile:domain` se il linguaggio non è stabile |
