...
Installi Guardrails in un progetto: la registrazione delle quattro
...
sappia come eseguire `gre-profile`, perché senza profilo di progetto le figure parlano per luoghi
...
4. Procedi senza domande di configurazione. La severità si deriva dalla criticità dichiarata nel
   ...
   il default operativo resta `normal` e il setup propone `gre-profile`.
...
- **Roster** → `{project-root}/_bmad/custom/config.toml`, una tabella `[agents.grl-agent-*]`
  per figura. È il passo che porta le quattro figure nel party mode: `resolve_party.py` costruisce
...
Devono comparire tutte e quattro le chiavi `grl-agent-*` accanto agli agenti già installati. Se
...
**non** la tabella degli agenti. Su un'installazione YAML le quattro figure vanno quindi registrate
...
- **Non crea `{project-root}/_bmad/memory/grl-shared/`.** La crea `gre-profile` alla prima
  esecuzione, quando ha qualcosa da scriverci. Una cartella vuota in `_bmad/memory/` è rumore.
- **Non imposta una stanza di default.** Le quattro figure restano nella stanza principale insieme
  agli agenti BMM; in più `gre-setup` installa stanze tematiche richiamabili con
...
1. Mostra cosa è stato scritto: le quattro figure registrate (nome, icona, titolo), le voci di
   help aggiunte e i file toccati.
2. Mostra il `module_greeting` di `module.yaml`.
3. **Proponi `gre-profile` e, se l'utente accetta, eseguilo subito.** È il passo che rende utile
   ...
   quanto saranno severe tutte e quattro le figure. Se rifiuta, va bene: digli che ogni figura
